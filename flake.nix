          ...
          (
            { ... }:
            {
              services.larp-bot = {
                ...
                scenariosDir = ./scenarios;
                # Arms the mayor service on every station card; it only
                # starts where a flash recipe wrote his identity — the base
                # station's card alone (base-station.just), so he runs on
                # exactly one card.
