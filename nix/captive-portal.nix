# Captive portal for AP mode, re-created at this level after the mailbox
# image dropped it ("2026-07-09b: drop range limiting … and the captive
# portal"). Same design as the removed upstream module, adapted to the new
# image; the option keeps the upstream name (dashchat.captivePortal) so the
# flake's per-station enable/disable wiring is unchanged.
#
# How the pieces fit:
#   * AP mode runs its own dnsmasq (dashchat-ap-dnsmasq, appliance.nix in the
#     mailbox image) reading /etc/dashchat-ap/dnsmasq.conf. The new image has
#     no conf-dir, so a wildcard `address=/#/` is appended (mkAfter) to that
#     file — it resolves EVERY name to the Pi, so the OS connectivity probes
...
#   * An iptables REDIRECT catches clients with hardcoded DNS servers
...
# hijacking DNS costs clients nothing — there is no internet to break. When
# the mesh is broadcast by a cabled access point (mAP lite, nix/base-station.nix)
# instead, that AP owns DHCP/DNS and this module stays inert.
{
  config,
  lib,
  pkgs,
...
in
{
  options.dashchat.captivePortal = {
    ...
    package = lib.mkOption {
      ...
      # The mayor's onboarding page (portal/index.html — a single static
      # file, no build step). Upstream's default was its generic Svelte SPA;
      # in this repo the mayor page is the only portal there is.
      default = pkgs.runCommand "mayor-portal" { } ''
        mkdir -p $out
        cp ${../portal/index.html} $out/index.html
      '';
      description = "Portal static files for nginx to serve.";
    };
  };

  config = lib.mkIf cfg.enable {
    # Resolve every DNS name to the Pi for AP clients.
    environment.etc."dashchat-ap/dnsmasq.conf".text = lib.mkAfter ''
      address=/#/${apAddress}
    '';
